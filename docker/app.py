...
class Predict:
    ...
    def handle_request(self, request: dict):
        ...
        if self.check_cache(self.result_connection, request["domain"]):
            return self.fetch_result(request["domain"]), falcon.HTTP_200
        else:
            if not self.check_cache(self.analysis_connection, request["domain"]):
                return self.create_task(request["domain"])
            else:
