...
class Evaluator:
    def __init__(self, data: list, domain: str, scaler, dense_model
                 # knn, linersvc, rforrest, lightgbm
                 ):
        self.domain = domain
    ...
    def predict_domain(self, model_name: str,) -> list:
        try:
    ...
    def predict_label(self):
        results = []
        for model_name in ["knn", "linearsvc", "lightgbm","rforest"]:
            if model_name != "lightgbm":
                results.extend(self.predict_domain(model_name))
            else:
                prediction = self.predict_domain(model_name)
